...
> Smart-Refactor - Open source tool by AetherCodeHQ

`cli` `code-quality` `developer-tools` `golang` `refactoring`

## What is it?

**Smart Refactor** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Smart-Refactor.git
cd Smart-Refactor
go build -o Smart-Refactor .
```
...
```bash
# Basic usage
./Smart-Refactor --path ./target

# With options
./Smart-Refactor --path ./target --format json --output report.json

# Verbose mode
./Smart-Refactor --path ./target --verbose
```
...
### Basic scan
```bash
./Smart-Refactor --path ./my-project
```

### JSON report
```bash
./Smart-Refactor --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Smart Refactor
  run: ./Smart-Refactor --path . --format json --output report.json
```
